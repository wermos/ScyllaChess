...
constexpr std::size_t diagonal_index(File file, Rank rank) noexcept {
    // file, rank both 0-based internally, but .file() returns File(char) and .rank() returns Rank(uint8)
    std::size_t file_idx = static_cast<std::size_t>(file); // 0..7
    std::size_t rank_idx = static_cast<std::size_t>(rank); // 0..7 (since Rank(1)=0)
